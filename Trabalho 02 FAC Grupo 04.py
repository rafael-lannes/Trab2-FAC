...
def mantissa_denormalized(entrada):#Função que trata dos casos não normalizados.
    separado=entrada.split(',')#Separa a entrada em dois segmentos.
    manti=separado[1][-23:]#Conta apenas a mantissa possível num caso não normalizado.
    return manti
...
def casos_especiais(entrada):#Imprime no formato IEEE754 e Retorna True se for um caso especial.
    ...
    if entrada=='0' or entrada=='+0':
        print('Caso Especial!! SINAL: 0 EXPOENTE: 00000000 MANTISSA: 00000000000000000000000')
        return True
    elif entrada=='-0':
        print('Caso Especial!! SINAL: 1 EXPOENTE: 00000000 MANTISSA: 00000000000000000000000')
        return True
    elif entrada=='+inf':
        print('Caso Especial!! SINAL: 0 EXPOENTE: 11111111 MANTISSA: 00000000000000000000000')
        return True
    elif entrada=='-inf':
        print('Caso Especial!! SINAL: 1 EXPOENTE: 11111111 MANTISSA: 00000000000000000000000')
        return True
    elif len(separado)>1:
        if len(separado[1])>=126:
            print('Valor Não Normalizado!! SINAL:',bit_sinal(entrada),' EXPOENTE: 00000000 MANTISSA:', mantissa_denormalized(remove_sinal(entrada)))
            return True
        else:
            return False
    elif entrada=='1':
...
def mantissa(entrada):#Função que retorna a Mantissa no formato Precisão simples (23 bits)
    ...
    if '1' in separado[0]:#Se a virgula anda para a Esquerda:
        ...
        for i in range(len(manti)):#Preenche o vetor da mantissa com os bits:
...
#Inicio do Programa:
print('-====Representação em numero Flutuante IEEE 754 Precisão Simples.====-')
...
#print(bit_sinal(entrada))
#print(expoente(entrada))
#print(mantissa(entrada))
